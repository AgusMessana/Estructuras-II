...
testLengthEmptySeq =
  TestCase $
    assertEqual
      "Error on empty sequence length"
      0
      (lengthS s0)
...
testLengthNonEmptySeq =
  TestCase $
    assertEqual
      "Error on non-empty sequence length"
      2
      (lengthS s2)
...
testMapNonEmptySeq =
  TestCase $
    assertEqual
      "Error on non-empty sequence map"
      (fromList [7, 4, 5])
      (mapS (+ 1) s3)
...
testReduceSumSeq3 =
  TestCase $
    assertEqual
      "Error reducing sequence of length 3"
      13
      (reduceS (+) 0 s3)
...
testScanSumSeq3 =
  TestCase $
    assertEqual
      "Error on scan for sequence of length 3"
      (fromList [0, 6, 9], 13)
      (scanS (+) 0 s3)
